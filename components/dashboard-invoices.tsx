	...
	return (
		<DashboardCard className="relative gap-0 md:col-span-2">
			<CardHeader className="border-b">
				...
				<CardDescription className="text-base">Open amounts and payment status.</CardDescription>
			</CardHeader>
			<CardContent className="mask-b-from-50% mask-b-to-100% px-0">
				<Table>
					...
					<TableHeader>
						<TableRow>
							<TableHead className="ps-6 text-sm">Customer</TableHead>
					...
					<TableBody>
						{invoices.map((inv) => (
							<TableRow className="h-14" key={inv.id}>
								<TableCell className="max-w-40 truncate ps-6 text-base font-medium">
									{inv.customer}
								</TableCell>
								<TableCell className="text-base text-muted-foreground tabular-nums">
									#{inv.id}
								</TableCell>
								<TableCell className="pe-6 text-right text-base tabular-nums">
									{inv.amount}
								</TableCell>
							</TableRow>
						))}
			...
			<div className="mask-t-from-30% absolute inset-x-0 bottom-0 flex h-1/5 items-center justify-center bg-background">
				<Button className="relative" variant="ghost" render={<Link href="/#" />} nativeButton={false}>View All
